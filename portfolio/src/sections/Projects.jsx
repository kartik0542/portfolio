...
const useIsMobile = (query = "(max-width: 639px)") => {
  ...
  useEffect(() => {
    ...
    const mql = window.matchMedia(query); //hamare window ka viewport ks resize ho raha hai ya nahi check kar raha hai
    const handler = (e) => setIsMobile(e.matches); // jab media query ki state change hogi tab run hoga fun

    mql.addEventListener("change", handler); //agar change thay to handler run thase
    setIsMobile(mql.matches); //first time user aayega tab set kar dega ki user ke pass laptop hai ya mobile

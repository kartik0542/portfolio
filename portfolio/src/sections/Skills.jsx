...
export default function Skills() {
  ...
  const [dir, setDir] = useState(-1); // icon right se left
  const [active, setActive] = useState(false);
  const sectionRef = useRef(null); // pure section ke liye
  const trackRef = useRef(null); // icons move section ke liye
  const touchY = useRef(null); // mobile display ke liye touch
  const x = useMotionValue(0); // horizontal move icon

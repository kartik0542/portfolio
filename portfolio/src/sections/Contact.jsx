...
export default function Contact() {
  ...
  const handleChange = (e) => {
    const { name, value } = e.target; // kis field ki name ki bat ho rahi hai
    if (name === "budget" && value && !/^\d+$/.test(value)) return; // sirf number hi return hoga
    setFormData((p) => ({ ...p, [name]: value })); // wahi value ko update karenge jo change hui hai baki ki update nahi karenge
    if (errors[name]) setErrors((p) => ({ ...p, [name]: "" }));
  };

  //jab tak sab fill nahi hoga tab tak validate karega
  const validateForm = () => {
    ...
    setErrors(newErrors);
    return !Object.keys(newErrors).length; // errors ki length batayegi
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await emailjs.send(
        ...
        TEMPLATE_ID,
        {
          // jo email likha hai wo ham tak pahuche
          ...formData,
  ...
  return (
    <section
      id="contact"
      className="w-full min-h-screen relative bg-black overflow-hidden text-white py-20 px-6 md:px-20 flex-col
    md:flex-row items-center  gap-10"
    >
      <ParticlesBackground />

      <div className="relative z-10 max-w-6xl w-full mx-auto flex flex-col md:flex-row items-center gap-10 ">
        {/* left side */}
        <motion.div
        ...
        >
          <motion.img
            ...
            animate={{ y: [0, -10, 0] }} // image movement
          ...
          <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
            <div className="flex flex-col ">
              ...
              <input
                ...
                //jo error hase to box red thase niche mujab
                className={`p-3 rounded-md bg-white/10 border ${
                  errors.name ? "border-red-500" : "border-gray-500"
                } text-white focus:outline-none focus:border-blue-500`}
              />
              {errors.name && ( // error hai to line run hogi
            ...
            <div className="flex flex-col">
              ...
              />
              {errors.email && ( // error hai to line run hogi
            ...
            {formData.service && formData.service !== "other" && (
              <div className="flex flex-col">
                ...
                />
                {errors.budget && ( // error hai to line run hogi
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              disabled={status === "sending"} // jyare all message send thay tyare button disable thay
              type="submit"
            >

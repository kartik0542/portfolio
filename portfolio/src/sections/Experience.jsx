...
function ExperienceItem({ exp, idx, start, end, scrollYProgress, layout }) {
  ...
  if (layout === "desktop") {
    return (
      <div className="relative flex flex-1 justify-between items-center min-w-0">
        <motion.div
        ...
        >
          ...
          <p className="text-md text-gray-300 break-word">{exp.description}</p>
        </motion.article>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex item-start">
      <motion.div
        className="absolute -left-3.5 top-3 z-10 w-7 h-7 rounded-full
       bg-white shadow-[0_0_0_8px_rgba(255, 255, 255, 0.1)]"
        style={{ scale, opacity }}
      ></motion.div>
      <motion.article
        className="bg-gray-900/80 backdrop-blur
       border-gray-700/70 rounded-xl p-5 w-[90vw] max-w-sm ml-6 shadow-lg"
        style={{ opacity, x }}
        transition={{ duration: 0.4, delay: 0.15 }}
      >
        <h3 className="text-lg font-semibold break-word">{exp.role}</h3>
        <p className="text-sm text-gray-300 mb-2 break-word">
          {exp.company} | {exp.duration}
        </p>
